...
<form class="cart grouped_form col-12 col-lg-9 mb-3" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
	<ul class="woocommerce-grouped-product-list list-group list-group-flush">
			...
			foreach ( $grouped_products as $grouped_product ) {
				...
				echo '<li id="product-' . esc_attr( $grouped_product->get_id() ) . '" class="woocommerce-grouped-product-list-item list-group-item px-0 ' . esc_attr( implode( ' ', get_post_class( '', $grouped_product->get_id() ) ) ) . '">';
				...
				foreach ( $grouped_product_columns as $column_id ) {
					...
					switch ( $column_id ) {
						...
						case 'label':
							...
							$value .= $grouped_product->is_visible() ? '<a href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', $grouped_product->get_permalink(), $grouped_product->get_id() ) ) . '" class="text-muted text-uppercase">' . $grouped_product->get_name() . '</a>' : $grouped_product->get_name();
							$value .= '</label>';
							break;
						case 'price':
	...
	<?php if ( $quantites_required ) : ?>
		...
		<button type="submit" class="single_add_to_cart_button button alt btn btn-dark btn-block text-uppercase mt-3">
			<span class="add_to_cart_label"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></span>
			<span class="loader d-none" aria-label="<?php echo esc_attr( 'Loading' ); ?>"></span>
		</button>
